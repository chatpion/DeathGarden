...
namespace TilemapJam {
	...
	public class CustomActorRenderer : Renderer {
		...
		/// <summary>
		/// [GET / SET] The depth offset for the <see cref="CustomActorRenderer"/> that is added
		...
		private void PrepareVertices (ref VertexC1P3T2[] vertices, IDrawDevice device, ColorRgba mainClr, Rect uvRect) {
			...
			if ((this.flip & SpriteRenderer.FlipMode.Horizontal) != SpriteRenderer.FlipMode.None) {
				edge1.X = -edge1.X;
				edge2.X = -edge2.X;
				edge3.X = -edge3.X;
				edge4.X = -edge4.X;
			}
			if ((this.flip & SpriteRenderer.FlipMode.Vertical) != SpriteRenderer.FlipMode.None) {
				edge1.Y = -edge1.Y;
				edge2.Y = -edge2.Y;
				edge3.Y = -edge3.Y;
				edge4.Y = -edge4.Y;
			...
			// Apply depth DepthOffsets
			float depthPerUnit = -this.DepthScale;
			if (this.IsVertical) {
				// Vertical actors share the same depth DepthOffset on all four vertices
				float baseDepthOffset = this.DepthOffset + transform.Pos.Y * depthPerUnit;
				...
				vertices[0].Pos.Z += this.DepthOffset + (worldBaseY + edge1.Y * transform.Scale / scaleTemp + this.Height) * depthPerUnit;
				vertices[1].Pos.Z += this.DepthOffset + (worldBaseY + edge2.Y * transform.Scale / scaleTemp + this.Height) * depthPerUnit;
				vertices[2].Pos.Z += this.DepthOffset + (worldBaseY + edge3.Y * transform.Scale / scaleTemp + this.Height) * depthPerUnit;
				vertices[3].Pos.Z += this.DepthOffset + (worldBaseY + edge4.Y * transform.Scale / scaleTemp + this.Height) * depthPerUnit;
			}
		}
